// Base de datos de posts del blog - Contenido optimizado para SEO
export const blogPosts = [
  {
    id: 1,
    slug: 'importancia-salud-bucal',
    title: '¿Por qué es importante la salud bucal?',
    excerpt: 'Descubre cómo mantener una buena salud bucal puede prevenir enfermedades y mejorar tu calidad de vida. Consejos prácticos de nuestros especialistas.',
    content: `
      <h2>La salud bucal: tu puerta de entrada a una vida saludable</h2>
      <p>La salud bucal es mucho más que tener una sonrisa bonita. Es fundamental para tu bienestar general y puede afectar directamente tu salud cardiovascular, digestiva y mental.</p>
      
      <h3>Beneficios de mantener una buena salud bucal</h3>
      <ul>
        <li><strong>Prevención de enfermedades:</strong> Una buena higiene bucal previene caries, gingivitis y periodontitis.</li>
        <li><strong>Mejora tu digestión:</strong> Masticar adecuadamente es el primer paso de una digestión saludable.</li>
        <li><strong>Aumenta tu confianza:</strong> Una sonrisa saludable mejora tu autoestima y relaciones sociales.</li>
        <li><strong>Protege tu corazón:</strong> Las enfermedades periodontales están relacionadas con problemas cardiovasculares.</li>
      </ul>
      
      <h3>Consejos para mantener tu salud bucal</h3>
      <ol>
        <li>Cepíllate los dientes 3 veces al día, después de cada comida</li>
        <li>Usa hilo dental diariamente para remover restos entre los dientes</li>
        <li>Visita al odontólogo cada 6 meses para chequeos preventivos</li>
        <li>Limita el consumo de azúcares y bebidas ácidas</li>
        <li>Reemplaza tu cepillo cada 3 meses</li>
      </ol>
      
      <p>En <strong>Periodent</strong>, nuestros especialistas están comprometidos con tu salud bucal integral. Agenda tu consulta y descubre cómo podemos ayudarte a mantener una sonrisa saludable.</p>
    `,
    image: '/img/importancia-salud-bucal.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-10',
    dateFormatted: '10 de Enero, 2026',
    category: 'Salud Dental',
    tags: ['salud bucal', 'prevención', 'higiene dental', 'consejos']
  },
  {
    id: 2,
    slug: 'ortodoncia-invisible-vs-tradicional',
    title: 'Ortodoncia invisible vs tradicional: ¿Cuál elegir?',
    excerpt: 'Comparamos las opciones de ortodoncia más populares para ayudarte a tomar la mejor decisión según tus necesidades y estilo de vida.',
    content: `
      <h2>Ortodoncia: Encuentra la mejor opción para ti</h2>
      <p>Elegir el tipo de ortodoncia adecuado puede ser un desafío. En este artículo comparamos las opciones más populares para que tomes una decisión informada.</p>
      
      <h3>Ortodoncia Tradicional (Brackets Metálicos)</h3>
      <p><strong>Ventajas:</strong></p>
      <ul>
        <li>Más económica que otras opciones</li>
        <li>Altamente efectiva para casos complejos</li>
        <li>No requiere tanta disciplina del paciente</li>
        <li>Tratamiento probado y confiable</li>
      </ul>
      
      <p><strong>Desventajas:</strong></p>
      <ul>
        <li>Visualmente más notoria</li>
        <li>Puede causar molestias iniciales</li>
        <li>Requiere cuidados especiales al comer</li>
      </ul>
      
      <h3>Ortodoncia Invisible (Alineadores Transparentes)</h3>
      <p><strong>Ventajas:</strong></p>
      <ul>
        <li>Estética: prácticamente invisible</li>
        <li>Removible para comer y cepillarse</li>
        <li>Más cómoda y menos irritante</li>
        <li>Ideal para adultos profesionales</li>
      </ul>
      
      <p><strong>Desventajas:</strong></p>
      <ul>
        <li>Más costosa que la tradicional</li>
        <li>Requiere disciplina para usarla 22 horas al día</li>
        <li>No apta para todos los casos complejos</li>
      </ul>
      
      <h3>¿Cuál es mejor para ti?</h3>
      <p>La elección depende de varios factores: complejidad del caso, presupuesto, estilo de vida y preferencias estéticas. En <strong>Periodent</strong>, nuestros ortodoncistas realizan un diagnóstico completo para recomendarte la mejor opción.</p>
      
      <p><strong>Agenda tu evaluación gratuita</strong> y descubre qué tipo de ortodoncia se adapta mejor a tus necesidades.</p>
    `,
    image: '/img/ortodoncia-invisible-vs-tradicional.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-12',
    dateFormatted: '12 de Enero, 2026',
    category: 'Ortodoncia',
    tags: ['ortodoncia', 'brackets', 'alineadores', 'invisalign']
  },
  {
    id: 3,
    slug: 'implantes-dentales-todo-lo-que-debes-saber',
    title: 'Implantes dentales: Todo lo que debes saber',
    excerpt: 'Guía completa sobre implantes dentales: beneficios, proceso, cuidados y por qué son la mejor solución para reemplazar dientes perdidos.',
    content: `
      <h2>Implantes Dentales: La solución permanente a la pérdida de dientes</h2>
      <p>Los implantes dentales son la solución más avanzada y duradera para reemplazar dientes perdidos. Descubre todo lo que necesitas saber antes de tomar esta decisión.</p>
      
      <h3>¿Qué son los implantes dentales?</h3>
      <p>Un implante dental es un pequeño tornillo de titanio que se coloca en el hueso maxilar para reemplazar la raíz del diente perdido. Sobre este implante se coloca una corona que simula perfectamente un diente natural.</p>
      
      <h3>Beneficios de los implantes dentales</h3>
      <ul>
        <li><strong>Aspecto natural:</strong> Se ven y se sienten como tus propios dientes</li>
        <li><strong>Durabilidad:</strong> Con los cuidados adecuados, pueden durar toda la vida</li>
        <li><strong>Funcionalidad:</strong> Puede ayudar a recuperar una masticación más cómoda, según la valoración y el tratamiento indicado</li>
        <li><strong>Protegen el hueso:</strong> Previenen la pérdida ósea que ocurre tras perder un diente</li>
        <li><strong>No afectan otros dientes:</strong> A diferencia de los puentes, no requieren tallar dientes adyacentes</li>
      </ul>
      
      <h3>El proceso de colocación</h3>
      <ol>
        <li><strong>Evaluación inicial:</strong> Radiografías y estudio de tu caso</li>
        <li><strong>Colocación del implante:</strong> Procedimiento quirúrgico mínimamente invasivo</li>
        <li><strong>Osteointegración:</strong> Periodo de 3-6 meses donde el implante se fusiona con el hueso</li>
        <li><strong>Colocación de la corona:</strong> Se coloca el diente definitivo</li>
      </ol>
      
      <h3>Cuidados post-operatorios</h3>
      <p>Los cuidados son simples: mantén una buena higiene bucal, evita fumar, y acude a tus citas de seguimiento. El éxito de los implantes supera el 95% con los cuidados adecuados.</p>
      
      <h3>¿Eres candidato para implantes?</h3>
      <p>La mayoría de personas son candidatas, pero requieres tener suficiente masa ósea y encías saludables. En <strong>Periodent</strong> realizamos un estudio completo para determinar tu elegibilidad.</p>
      
      <p><strong>Agenda tu consulta de evaluación</strong> y recupera tu sonrisa con la tecnología más avanzada.</p>
    `,
    image: '/img/implantes-dentales-todo-lo-que-debes-saber.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-14',
    dateFormatted: '14 de Enero, 2026',
    category: 'Implantología',
    tags: ['implantes dentales', 'prótesis', 'cirugía dental', 'reemplazo dental']
  },
  {
    id: 4,
    slug: 'limpieza-dental-profesional-cada-cuanto',
    title: '¿Cada cuánto debo hacerme una limpieza dental profesional?',
    excerpt: 'Aprende la frecuencia ideal para realizar limpiezas dentales profesionales y cómo este procedimiento protege tu salud bucal a largo plazo.',
    content: `
      <h2>La importancia de la limpieza dental profesional</h2>
      <p>Aunque te cepilles y uses hilo dental religiosamente, necesitas limpiezas profesionales regulares. Descubre por qué y con qué frecuencia.</p>
      
      <h3>¿Qué es una limpieza dental profesional?</h3>
      <p>También conocida como profilaxis, es un procedimiento realizado por un odontólogo o higienista dental para remover placa bacteriana, sarro y manchas que no pueden eliminarse con el cepillado diario.</p>
      
      <h3>Frecuencia recomendada</h3>
      <p><strong>Para la mayoría de personas:</strong> Cada 6 meses</p>
      <p><strong>Si tienes mayor riesgo:</strong> Cada 3-4 meses si presentas:</p>
      <ul>
        <li>Enfermedad periodontal (gingivitis o periodontitis)</li>
        <li>Diabetes</li>
        <li>Tabaquismo</li>
        <li>Sistema inmune comprometido</li>
        <li>Tendencia a acumular sarro rápidamente</li>
      </ul>
      
      <h3>Beneficios de las limpiezas regulares</h3>
      <ul>
        <li><strong>Prevención de caries:</strong> Remueve la placa antes de que cause daño</li>
        <li><strong>Prevención de enfermedades de encías:</strong> Elimina el sarro que causa inflamación</li>
        <li><strong>Detección temprana:</strong> Tu dentista puede identificar problemas antes de que empeoren</li>
        <li><strong>Aliento fresco:</strong> Elimina bacterias que causan mal aliento</li>
        <li><strong>Sonrisa más brillante:</strong> Remueve manchas de café, té y otros alimentos</li>
      </ul>
      
      <h3>¿Qué esperar durante el procedimiento?</h3>
      <ol>
        <li>Examen bucal completo</li>
        <li>Remoción de sarro con instrumentos especiales</li>
        <li>Pulido de los dientes</li>
        <li>Aplicación de flúor (opcional)</li>
        <li>Recomendaciones personalizadas</li>
      </ol>
      
      <p>El procedimiento es indoloro y dura aproximadamente 30-45 minutos.</p>
      
      <h3>Invierte en prevención</h3>
      <p>Las limpiezas dentales regulares son una inversión en tu salud. Prevenir es mucho más económico que tratar enfermedades avanzadas.</p>
      
      <p>En <strong>Periodent</strong>, nuestras limpiezas incluyen evaluación completa y asesoría personalizada. <strong>Agenda tu próxima limpieza</strong> y mantén tu sonrisa saludable.</p>
    `,
    image: '/img/limpieza-dental-profesional-cada-cuanto.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-16',
    dateFormatted: '16 de Enero, 2026',
    category: 'Prevención',
    tags: ['limpieza dental', 'profilaxis', 'prevención', 'higiene dental']
  },
  {
    id: 5,
    slug: 'clinica-dental-chabasquen-portuguesa',
    title: 'Clínica Dental en Chabasquén, Portuguesa: Tu Sonrisa en Manos Expertas',
    excerpt: 'Descubre por qué Periodent es la clínica odontológica de confianza en Chabasquén y toda Portuguesa. Tecnología avanzada y atención personalizada cerca de ti.',
    content: `
      <h2>Tu Clínica Dental de Confianza en Chabasquén</h2>
      <p>En <strong>Periodent</strong>, ubicados en el corazón de Chabasquén, Portuguesa, nos enorgullece ser la clínica odontológica de referencia en la región. Con años de experiencia y un equipo de especialistas altamente calificados, ofrecemos atención dental integral para toda tu familia.</p>
      
      <h3>¿Por qué elegir Periodent en Chabasquén?</h3>
      <ul>
        <li><strong>Ubicación céntrica:</strong> Nos encontramos en Avenida Sucre con Calle Cordoba y Arismendi, Edificio del Señor Chelopez, Planta baja, local 5, Chabasquén</li>
        <li><strong>Tecnología avanzada:</strong> Contamos con equipos de última generación para diagnósticos precisos y tratamientos efectivos</li>
        <li><strong>Equipo multidisciplinario:</strong> Ortodoncistas, implantólogos, periodoncistas y odontólogos generales</li>
        <li><strong>Atención personalizada:</strong> Cada paciente recibe un plan de tratamiento adaptado a sus necesidades</li>
        <li><strong>Horarios flexibles:</strong> Facilitamos tu visita con horarios que se ajustan a tu rutina</li>
      </ul>
      
      <h3>Servicios Dentales en Chabasquén, Portuguesa</h3>
      <p>En nuestra clínica dental de Chabasquén ofrecemos una amplia gama de servicios:</p>
      <ul>
        <li><strong>Ortodoncia:</strong> Brackets tradicionales y sistemas invisibles</li>
        <li><strong>Implantes dentales:</strong> Soluciones permanentes para dientes perdidos</li>
        <li><strong>Limpieza y profilaxis:</strong> Prevención de enfermedades bucales</li>
        <li><strong>Estética dental:</strong> Blanqueamiento, carillas y diseño de sonrisa</li>
        <li><strong>Endodoncia:</strong> Tratamiento de conductos con técnicas de control del dolor</li>
        <li><strong>Odontopediatría:</strong> Atención especializada para los más pequeños</li>
      </ul>
      
      <h3>Sirviendo a la Comunidad de Portuguesa</h3>
      <p>Aunque estamos ubicados en Chabasquén, atendemos pacientes de toda Portuguesa, incluyendo Biscucuy, Guanare, Acarigua y municipios cercanos. Nuestra ubicación estratégica cerca de la plaza Bolívar facilita el acceso desde cualquier punto de la región.</p>
      
      <h3>Agenda tu Cita en Periodent Chabasquén</h3>
      <p>¿Necesitas atención odontológica en Chabasquén? Nuestro equipo está listo para ayudarte. Ofrecemos:</p>
      <ul>
        <li>Evaluación inicial; consulta las condiciones vigentes al solicitar tu cita</li>
        <li>Planes de tratamiento flexibles</li>
        <li>Facilidades de pago</li>
        <li>Atención de emergencias dentales</li>
      </ul>
      
      <p><strong>Visítanos en:</strong> Avenida Sucre con Calle Cordoba y Arismendi, Edificio del Señor Chelopez, Planta baja, local 5, Chabasquén-Portuguesa.</p>
      <p><strong>Llámanos:</strong> +58 (412) 155 35 98</p>
      
      <p>En <strong>Periodent Chabasquén</strong>, tu sonrisa es nuestra prioridad. ¡Te esperamos!</p>
    `,
    image: '/img/clinica-dental-chabasquen-portuguesa.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-15',
    dateFormatted: '15 de Enero, 2026',
    category: 'Clínica',
    tags: ['Chabasquén', 'Portuguesa', 'clínica dental', 'servicios dentales', 'odontología']
  },
  {
    id: 6,
    slug: 'ortodoncia-en-portuguesa-venezuela',
    title: 'Ortodoncia en Portuguesa: Mejores Opciones y Precios Accesibles',
    excerpt: 'Buscas ortodoncia en Portuguesa? Descubre las mejores opciones de tratamiento ortodóntico en Chabasquén con precios competitivos y especialistas certificados.',
    content: `
      <h2>Ortodoncia de Calidad en Portuguesa, Venezuela</h2>
      <p>Si buscas un tratamiento de ortodoncia en Portuguesa, <strong>Periodent</strong> en Chabasquén es tu mejor opción. Contamos con ortodoncistas certificados y amplia experiencia tratando pacientes de toda la región.</p>
      
      <h3>¿Por qué hacer tu ortodoncia en Periodent Chabasquén?</h3>
      <ul>
        <li><strong>Especialistas certificados:</strong> Nuestros ortodoncistas están altamente capacitados y actualizados en las últimas técnicas</li>
        <li><strong>Tecnología 3D:</strong> Diagnóstico preciso con radiografías digitales y planificación virtual</li>
        <li><strong>Múltiples opciones:</strong> Desde brackets metálicos hasta sistemas invisibles</li>
        <li><strong>Precios accesibles:</strong> Planes de pago flexibles adaptados a tu presupuesto</li>
        <li><strong>Seguimiento personalizado:</strong> Acompañamiento durante todo tu tratamiento</li>
      </ul>
      
      <h3>Tipos de Ortodoncia Disponibles en Chabasquén</h3>
      
      <h4>Ortodoncia Tradicional con Brackets</h4>
      <p>Los brackets metálicos siguen siendo una excelente opción, especialmente para casos complejos. Son los más económicos y altamente efectivos.</p>
      <p><strong>Ideal para:</strong> Niños, adolescentes y adultos con cualquier tipo de malposición dental.</p>
      
      <h4>Brackets Estéticos</h4>
      <p>Fabricados en cerámica o zafiro, son menos visibles que los metálicos. Perfectos si buscas una opción más discreta.</p>
      <p><strong>Ideal para:</strong> Adultos profesionales y personas que valoran la estética.</p>
      
      <h4>Ortodoncia Invisible</h4>
      <p>Alineadores transparentes removibles. La opción más estética y cómoda del mercado.</p>
      <p><strong>Ideal para:</strong> Adultos con casos de leves a moderados que buscan discreción absoluta.</p>
      
      <h3>Proceso de Ortodoncia en Periodent</h3>
      <ol>
        <li><strong>Evaluación inicial gratuita:</strong> Analizamos tu caso y te explicamos las opciones</li>
        <li><strong>Diagnóstico completo:</strong> Radiografías, fotos y modelos de estudio</li>
        <li><strong>Plan de tratamiento:</strong> Te presentamos el plan con tiempos y costos</li>
        <li><strong>Colocación:</strong> Instalamos tu aparato ortodóntico</li>
        <li><strong>Controles mensuales:</strong> Seguimiento y ajustes periódicos</li>
        <li><strong>Retenedores:</strong> Al finalizar, mantenemos tu sonrisa perfecta</li>
      </ol>
      
      <h3>Atendemos Pacientes de Toda Portuguesa</h3>
      <p>Aunque nuestra clínica está en Chabasquén, recibimos pacientes de:</p>
      <ul>
        <li>Biscucuy</li>
        <li>Guanare</li>
        <li>Acarigua-Araure</li>
        <li>Ospino</li>
        <li>Guanarito</li>
        <li>Y todos los municipios de Portuguesa</li>
      </ul>
      
      <h3>Precios de Ortodoncia en Portuguesa</h3>
      <p>En <strong>Periodent</strong> ofrecemos precios competitivos con planes de financiamiento. El costo varía según el tipo de ortodoncia y la complejidad del caso.</p>
      <p>Solicita tu evaluación gratuita y recibe un presupuesto personalizado sin compromiso.</p>
      
      <p><strong>Agenda tu consulta de ortodoncia en Chabasquén:</strong> +58 (412) 155 35 98</p>
      <p>Avenida Sucre con Calle Cordoba y Arismendi, Edificio del Señor Chelopez, Planta baja, local 5, Chabasquén-Portuguesa.</p>
    `,
    image: '/img/ortodoncia-en-portuguesa-venezuela.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-13',
    dateFormatted: '13 de Enero, 2026',
    category: 'Ortodoncia',
    tags: ['ortodoncia Portuguesa', 'brackets Chabasquén', 'ortodoncista', 'Venezuela']
  },
  {
    id: 7,
    slug: 'implantes-dentales-portuguesa-chabasquen',
    title: 'Implantes Dentales en Portuguesa: Recupera tu Sonrisa con Periodent',
    excerpt: 'Los mejores implantes dentales en Portuguesa con garantía. Especialistas en implantología en Chabasquén con tecnología de vanguardia y precios justos.',
    content: `
      <h2>Implantes Dentales en Chabasquén, Portuguesa</h2>
      <p>Perder uno o varios dientes afecta tu salud, estética y autoestima. En <strong>Periodent Chabasquén</strong>, somos especialistas en implantología dental, ofreciendo soluciones permanentes y naturales para recuperar tu sonrisa.</p>
      
      <h3>¿Qué son los Implantes Dentales?</h3>
      <p>Un implante dental es un tornillo de titanio que se coloca en el hueso maxilar, funcionando como la raíz de un diente natural. Sobre este implante se coloca una corona que se ve y funciona exactamente como un diente real.</p>
      
      <h3>Ventajas de los Implantes Dentales en Periodent</h3>
      <ul>
        <li><strong>Implantólogo certificado:</strong> Nuestro especialista cuenta con formación avanzada y años de experiencia</li>
        <li><strong>Implantes de titanio de grado médico:</strong> Materiales de la más alta calidad con garantía</li>
        <li><strong>Tecnología 3D:</strong> Planificación digital para implantes precisos y seguros</li>
        <li><strong>Procedimiento mínimamente invasivo:</strong> Técnicas modernas con recuperación rápida</li>
        <li><strong>Resultados naturales:</strong> Coronas personalizadas que combinan perfectamente con tus dientes</li>
      </ul>
      
      <h3>Tipos de Tratamientos con Implantes</h3>
      
      <h4>Implante Unitario</h4>
      <p>Para reemplazar un solo diente perdido. Solución ideal que no afecta los dientes adyacentes.</p>
      <p><strong>Duración del tratamiento:</strong> 3-6 meses (incluye osteointegración)</p>
      
      <h4>Puente sobre Implantes</h4>
      <p>Para reemplazar varios dientes contiguos usando 2 o más implantes como soporte.</p>
      <p><strong>Ventaja:</strong> Más estable y duradero que un puente tradicional</p>
      
      <h4>Prótesis Completa sobre Implantes</h4>
      <p>Para pacientes sin dientes. Se colocan 4-6 implantes que sostienen una prótesis fija completa.</p>
      <p><strong>Técnica All-on-4:</strong> Recupera todos tus dientes con solo 4 implantes por arcada</p>
      
      <h3>Proceso de Colocación en Periodent Chabasquén</h3>
      <ol>
        <li><strong>Evaluación inicial:</strong> Examen clínico y radiografías 3D</li>
        <li><strong>Planificación digital:</strong> Diseño del tratamiento con software especializado</li>
        <li><strong>Cirugía de colocación:</strong> Procedimiento con anestesia local y seguimiento profesional</li>
        <li><strong>Osteointegración:</strong> Periodo de 3-6 meses donde el implante se fusiona con el hueso</li>
        <li><strong>Colocación de la corona:</strong> Se instala el diente definitivo</li>
      </ol>
      
      <h3>¿Eres Candidato para Implantes?</h3>
      <p>La mayoría de personas pueden recibir implantes dentales. Requisitos básicos:</p>
      <ul>
        <li>Suficiente masa ósea (o posibilidad de injerto óseo)</li>
        <li>Encías saludables o tratables</li>
        <li>Buena salud general</li>
        <li>No fumar o disposición a dejarlo durante el tratamiento</li>
      </ul>
      
      <h3>Implantes Dentales en Portuguesa: Tu Mejor Inversión</h3>
      <p>Un implante dental bien colocado puede durar toda la vida. Es la solución más duradera y natural para reemplazar dientes perdidos.</p>
      
      <h3>¿Por qué Pacientes de Toda Portuguesa Eligen Periodent?</h3>
      <ul>
        <li>Especialista con más de 1000 implantes colocados</li>
        <li>Tecnología de vanguardia en Chabasquén</li>
        <li>Precios competitivos con planes de pago</li>
        <li>Garantía en implantes y coronas</li>
        <li>Atención personalizada y seguimiento continuo</li>
      </ul>
      
      <p>Recibimos pacientes de Biscucuy, Guanare, Acarigua, Ospino y toda Portuguesa.</p>
      
      <h3>Agenda tu Evaluación de Implantes</h3>
      <p><strong>Consulta inicial sin costo.</strong> Te realizaremos un estudio completo y te presentaremos tu plan de tratamiento con presupuesto detallado.</p>
      
      <p><strong>Periodent Chabasquén</strong><br>
      Avenida Sucre con Calle Cordoba y Arismendi,<br>
      Edificio del Señor Chelopez<br>
      Planta baja, local 5<br>
      Chabasquén, Portuguesa<br>
      <strong>Teléfono:</strong> +58 (412) 155 35 98</p>
      
      <p>¡Recupera tu sonrisa con los mejores implantólogos de Portuguesa!</p>
    `,
    image: '/img/implantes-dentales-portuguesa-chabasquen.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-11',
    dateFormatted: '11 de Enero, 2026',
    category: 'Implantología',
    tags: ['implantes dentales Portuguesa', 'implantólogo Chabasquén', 'All-on-4', 'Venezuela']
  },
  {
    id: 8,
    slug: 'clinica-dental-biscucuy-portuguesa',
    title: 'Clínica Dental en Biscucuy, Portuguesa: Especialistas Odontológicos a tu Servicio',
    excerpt: 'Visita Periodent en Biscucuy para recibir atención odontológica de calidad. Servicios completos, especialistas certificados y tecnología moderna en tu localidad.',
    content: `
      <h2>Tu Clínica Dental en Biscucuy, Portuguesa</h2>
      <p>En <strong>Periodent Biscucuy</strong>, ubicados en la Carrera 1 Bolívar, somos tu clínica odontológica de confianza. Ofrecemos servicios integrales de salud dental con un equipo de especialistas comprometidos con tu bienestar.</p>
      
      <h3>¿Por qué Elegir Periodent en Biscucuy?</h3>
      <ul>
        <li><strong>Ubicación accesible:</strong> Carrera 1 Bolívar, entre calle Paez y Negro Primero, junto a la antigua Casa Blanca</li>
        <li><strong>Especialistas multidisciplinarios:</strong> Contamos con ortodoncistas, implantólogos y odontólogos generales</li>
        <li><strong>Tecnología de punta:</strong> Equipos modernos para diagnósticos precisos y tratamientos efectivos</li>
        <li><strong>Atención personalizada:</strong> Cada paciente recibe un plan adaptado a sus necesidades específicas</li>
        <li><strong>Precios competitivos:</strong> Ofrecemos planes de pago flexibles y accesibles</li>
      </ul>
      
      <h3>Servicios Odontológicos en Biscucuy</h3>
      <p>Somos una clínica completa que ofrece toda la gama de servicios dentales:</p>
      <ul>
        <li><strong>Odontología General:</strong> Tratamiento de caries, limpiezas y extracciones</li>
        <li><strong>Ortodoncia:</strong> Brackets y sistemas invisibles para alineación dental</li>
        <li><strong>Implantes Dentales:</strong> Reemplazo permanente de dientes perdidos</li>
        <li><strong>Estética Dental:</strong> Blanqueamiento, carillas y diseño de sonrisa</li>
        <li><strong>Endodoncia:</strong> Tratamientos de conductos con técnicas de control del dolor</li>
        <li><strong>Periodoncia:</strong> Tratamiento de encías y prevención de enfermedades</li>
        <li><strong>Odontopediatría:</strong> Atención especializada para niños</li>
      </ul>
      
      <h3>Biscucuy y Alrededores: Nuestro Área de Servicio</h3>
      <p>Aunque nuestra sucursal principal está en Biscucuy, también atendemos pacientes de:</p>
      <ul>
        <li>Chabasquén</li>
        <li>Guanare</li>
        <li>Acarigua-Araure</li>
        <li>Ospino</li>
        <li>Todos los municipios de Portuguesa</li>
      </ul>
      
      <h3>Primera Consulta Sin Costo</h3>
      <p>Ofrecemos evaluación inicial gratuita donde:</p>
      <ul>
        <li>Examinamos tu salud bucal completa</li>
        <li>Identificamos problemas dentales</li>
        <li>Te presentamos opciones de tratamiento</li>
        <li>Brindamos presupuesto detallado sin compromiso</li>
      </ul>
      
      <h3>¿Cómo Llegar a Periodent Biscucuy?</h3>
      <p><strong>Dirección:</strong> Carrera 1 Bolívar, entre calle Paez y Negro Primero, Al lado de la antigua Casa Blanca, Biscucuy-Portuguesa.</p>
      <p><strong>Teléfono:</strong> +58 (412) 155 35 98</p>
      <p><strong>Horarios:</strong> Lunes a Viernes 8:00 AM - 5:00 PM | Sábados 9:00 AM - 1:00 PM</p>
      
      <h3>¿Qué Dicen nuestros Pacientes?</h3>
      <p>Periodent Biscucuy es reconocida en la comunidad por la calidad de sus servicios, profesionalismo del equipo y atención al paciente.</p>
      
      <p><strong>Agenda tu cita hoy mismo</strong> y comienza tu camino hacia una sonrisa saludable. ¡Te esperamos en Periodent Biscucuy!</p>
    `,
    image: '/img/clinica-dental-biscucuy-portuguesa.png',
    author: 'Equipo Periodent',
    date: '2026-01-09',
    dateFormatted: '9 de Enero, 2026',
    category: 'Clínica',
    tags: ['Biscucuy', 'Portuguesa', 'odontólogo', 'clínica dental']
  },
  {
    id: 9,
    slug: 'ortodoncia-biscucuy-brackets-invisalign',
    title: 'Ortodoncia en Biscucuy: Brackets, Invisalign y Tratamientos Modernos',
    excerpt: 'Información sobre ortodoncia en Biscucuy, brackets y alineadores, con orientación para comparar alternativas y solicitar una evaluación.',
    content: `
      <h2>Tratamientos Ortodónticos en Biscucuy, Portuguesa</h2>
      <p>¿Buscas corregir la alineación de tus dientes? En <strong>Periodent Biscucuy</strong> encontrarás la solución perfecta con nuestros especialistas en ortodoncia y tecnología de vanguardia.</p>
      
      <h3>Especialista en Ortodoncia en Biscucuy</h3>
      <p>Nuestro ortodoncista certificado cuenta con:</p>
      <ul>
        <li>Especialización en ortodoncia moderna</li>
        <li>Años de experiencia tratando cientos de pacientes</li>
        <li>Actualización continua en nuevas técnicas</li>
        <li>Membresía en organizaciones ortodónticas profesionales</li>
      </ul>
      
      <h3>Opciones de Ortodoncia Disponibles</h3>
      
      <h4>Brackets Metálicos</h4>
      <p>La opción más económica y efectiva para cualquier tipo de malposición dental. Ideal para niños y adolescentes.</p>
      <ul>
        <li>Precio más accesible</li>
        <li>Efectivo para todos los casos</li>
        <li>Resultados rápidos</li>
      </ul>
      
      <h4>Brackets Estéticos (Cerámica)</h4>
      <p>Brackets de color blanco que se mimetizan con tus dientes. Perfectos para adultos que desean discreción.</p>
      <ul>
        <li>Menos visibles que los metálicos</li>
        <li>Muy resistentes</li>
        <li>Excelente relación precio-estética</li>
      </ul>
      
      <h4>Ortodoncia Invisible (Alineadores Transparentes)</h4>
      <p>Sistema de alineadores removibles. La opción más estética del mercado actual.</p>
      <ul>
        <li>Prácticamente invisible</li>
        <li>Se puede remover para comer y cepillarse</li>
        <li>Más cómoda que brackets</li>
        <li>Ideal para profesionales y adultos</li>
      </ul>
      
      <h3>¿Cuánto Tiempo Dura un Tratamiento de Ortodoncia?</h3>
      <ul>
        <li><strong>Casos simples:</strong> 6-12 meses</li>
        <li><strong>Casos moderados:</strong> 12-24 meses</li>
        <li><strong>Casos complejos:</strong> 24-36 meses</li>
      </ul>
      <p>El tiempo dependerá de la complejidad de tu caso y del tipo de aparatología que elijas.</p>
      
      <h3>Proceso de Ortodoncia en Biscucuy</h3>
      <ol>
        <li><strong>Evaluación inicial:</strong> Análisis de tu caso (sin costo)</li>
        <li><strong>Radiografías y fotos:</strong> Diagnóstico detallado</li>
        <li><strong>Moldes de estudio:</strong> Planificación del tratamiento</li>
        <li><strong>Colocación:</strong> Instalación del aparato ortodóntico</li>
        <li><strong>Seguimiento mensual:</strong> Controles y ajustes</li>
        <li><strong>Retención:</strong> Mantenimiento al finalizar</li>
      </ol>
      
      <h3>Precios de Ortodoncia en Biscucuy</h3>
      <p>En Periodent Biscucuy ofrecemos precios competitivos con opciones de financiamiento flexibles. Cada plan se personaliza según:</p>
      <ul>
        <li>Tipo de aparatología</li>
        <li>Complejidad del caso</li>
        <li>Duración estimada del tratamiento</li>
      </ul>
      
      <h3>¿Por qué Pacientes de Toda Portuguesa Eligen Periodent Biscucuy?</h3>
      <ul>
        <li>Especialista con amplia experiencia</li>
        <li>Tecnología 3D para diagnóstico preciso</li>
        <li>Múltiples opciones de aparatología</li>
        <li>Precios accesibles y planes de pago</li>
        <li>Ubicación céntrica en Biscucuy</li>
        <li>Atención personalizada y profesional</li>
      </ul>
      
      <h3>Agenda tu Consulta Ortodóntica Hoy</h3>
      <p><strong>Primera consulta sin costo.</strong> Evaluamos tu caso y brindamos opciones de tratamiento con presupuesto detallado.</p>
      
      <p><strong>Periodent Biscucuy</strong><br>
      Carrera 1 Bolívar, entre calle Paez y Negro Primero<br>
      Al lado de la antigua Casa Blanca<br>
      Biscucuy, Portuguesa<br>
      <strong>Teléfono:</strong> +58 (412) 155 35 98</p>
      
      <p>¡Consigue la sonrisa que siempre deseaste con ortodoncia en Biscucuy!</p>
    `,
    image: '/img/ortodoncia-biscucuy-brackets-invisalign.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-08',
    dateFormatted: '8 de Enero, 2026',
    category: 'Ortodoncia',
    tags: ['ortodoncia Biscucuy', 'brackets Portuguesa', 'invisalign', 'especialista']
  },
  {
    id: 10,
    slug: 'implantes-dentales-biscucuy-garantizados',
    title: 'Implantes Dentales en Biscucuy: Solución Duradera y Personalizada',
    excerpt: 'Información sobre implantes dentales en Biscucuy, evaluación del caso, planificación y cuidados posteriores.',
    content: `
      <h2>Implantes Dentales en Biscucuy</h2>
      <p>Los implantes dentales pueden ser una alternativa para reemplazar dientes perdidos. En <strong>Periodent Biscucuy</strong> evaluamos cada caso y explicamos las opciones, los beneficios, los riesgos y los cuidados necesarios.</p>
      
      <h3>¿Cómo se evalúa un tratamiento de implantes?</h3>
      <ul>
        <li><strong>Valoración profesional:</strong> Revisión de la salud bucal y de las condiciones del paciente</li>
        <li><strong>Materiales adecuados:</strong> Selección de componentes según el diagnóstico y el plan de tratamiento</li>
        <li><strong>Tecnología 3D:</strong> Tomografía digital para planificación precisa y segura</li>
        <li><strong>Cirugía mínimamente invasiva:</strong> Técnicas modernas con recuperación rápida</li>
        <li><strong>Coronas personalizadas:</strong> Hechas a medida para combinar perfectamente con tus dientes</li>
        <li><strong>Seguimiento:</strong> Controles para revisar la evolución y resolver dudas sobre los cuidados</li>
      </ul>
      
      <h3>¿Cuánto Duran los Implantes Dentales?</h3>
      <p>Con los cuidados adecuados, <strong>los implantes dentales pueden durar toda la vida</strong>. Son la inversión más duradera en salud dental.</p>
      <ul>
        <li>Longevidad: 20-30 años o más</li>
        <li>Tasa de éxito: 95-98%</li>
        <li>Resultado: Diente natural en apariencia y función</li>
      </ul>
      
      <h3>Tipos de Implantes que Ofrecemos</h3>
      
      <h4>Implante Unitario</h4>
      <p>Para reemplazar un solo diente. No requiere tallar los dientes adyacentes.</p>
      
      <h4>Puente sobre Implantes</h4>
      <p>Para varios dientes contiguos. Usa 2 o más implantes como soporte de una corona en puente.</p>
      
      <h4>Prótesis Completa sobre Implantes</h4>
      <p>Recupera todos tus dientes con la técnica All-on-4 o All-on-6.</p>
      
      <h3>Paso a Paso: Colocación de Implante Dental en Biscucuy</h3>
      <ol>
        <li><strong>Evaluación clínica:</strong> Examen y radiografías 3D</li>
        <li><strong>Análisis de densidad ósea:</strong> Determinamos si hay suficiente hueso (si no, hacemos injerto)</li>
        <li><strong>Planificación digital:</strong> Diseño exacto del implante en computadora</li>
        <li><strong>Colocación del implante:</strong> Cirugía bajo anestesia local y con indicaciones postoperatorias</li>
        <li><strong>Cicatrización (3-6 meses):</strong> El implante se integra con el hueso (osteointegración)</li>
        <li><strong>Colocación de corona:</strong> Se instala el diente artificial permanente</li>
      </ol>
      
      <h3>¿Duele la Colocación de un Implante?</h3>
      <p><strong>No.</strong> La cirugía se realiza bajo anestesia local. Durante el procedimiento no sentirás dolor, solo presión.</p>
      <p>Después de la cirugía puede haber molestias leves que se controlan fácilmente con analgésicos comunes.</p>
      
      <h3>Requisitos para Implantes Dentales</h3>
      <p>Aunque la mayoría de personas pueden recibir implantes, algunos requisitos incluyen:</p>
      <ul>
        <li>Encías saludables o tratables</li>
        <li>Altura y volumen óseo adecuado (o posibilidad de injerto)</li>
        <li>Buena salud general</li>
        <li>No fumar o disposición a dejar de fumar durante el tratamiento</li>
        <li>Higiene oral adecuada</li>
      </ul>
      
      <h3>Cuidados Posteriores a la Colocación</h3>
      <p>Para asegurar el éxito de tu implante:</p>
      <ul>
        <li>Mantén buena higiene oral (cepillado y hilo dental)</li>
        <li>Evita fumar y consumo excesivo de alcohol</li>
        <li>Asiste a controles periódicos</li>
        <li>Usa protector bucal si practicas deportes</li>
        <li>Evita morder objetos duros</li>
      </ul>
      
      <h3>Implantes Dentales en Biscucuy: Tu Mejor Inversión</h3>
      <p>Aunque el costo inicial puede parecer alto, los implantes son la inversión más rentable:</p>
      <ul>
        <li>Duran décadas (casi de por vida)</li>
        <li>No requieren mantenimiento especial</li>
        <li>Recuperas función y estética completas</li>
        <li>Mejora tu calidad de vida y autoestima</li>
      </ul>
      
      <h3>Opciones de Pago Flexibles</h3>
      <p>Ofrecemos planes de financiamiento accesibles para que puedas hacer tu implante sin afectar tu economía.</p>
      
      <h3>Consulta Gratuita de Implantes</h3>
      <p>Agenda tu evaluación sin costo. Te haremos un estudio completo (radiografías, análisis óseo) y te presentaremos tu plan de tratamiento personalizado con presupuesto detallado.</p>
      
      <p><strong>Periodent Biscucuy</strong><br>
      Carrera 1 Bolívar, entre calle Paez y Negro Primero<br>
      Al lado de la antigua Casa Blanca<br>
      Biscucuy, Portuguesa<br>
      <strong>Teléfono:</strong> +58 (412) 155 35 98</p>
      
      <p>Agenda una evaluación para conocer si los implantes dentales son adecuados para tu caso en Biscucuy.</p>
    `,
    image: '/img/implantes-dentales-biscucuy-garantizados.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-01-07',
    dateFormatted: '7 de Enero, 2026',
    category: 'Implantología',
    tags: ['implantes Biscucuy', 'cirugía dental', 'All-on-4', 'garantía']
  },
  {
    id: 11,
    slug: 'diseno-de-sonrisa-carillas-dentales',
    title: 'Diseño de Sonrisa y Carillas Dentales: Guía Completa de Estética Dental',
    excerpt: 'Descubre qué es el diseño de sonrisa, las diferencias entre carillas de porcelana y resina, y cómo transformar la estética de tus dientes con resultados naturales.',
    content: `
      <h2>Diseño de Sonrisa: Transforma tu Estética Dental</h2>
      <p>El diseño de sonrisa es un conjunto de procedimientos odontológicos personalizados destinados a mejorar la apariencia estética de tus dientes, encías y labia, logrando una armonía perfecta con las facciones de tu rostro.</p>
      
      <h3>¿Qué son las Carillas Dentales?</h3>
      <p>Las carillas dentales son láminas muy finas que se adhieren a la parte frontal de los dientes para corregir problemas de color, forma, tamaño o alineación leve.</p>
      
      <h3>Carillas de Porcelana vs. Carillas de Resina Compuesta</h3>
      
      <h4>Carillas de Porcelana (Cerámica)</h4>
      <p>Son láminas personalizadas confeccionadas en laboratorio dental con materiales cerámicos de alta resistencia.</p>
      <ul>
        <li><strong>Ventajas:</strong> Apariencia extremadamente natural, no se manchan con café o tabaco, durabilidad de 15 a 20 años.</li>
        <li><strong>Inconvenientes:</strong> Requieren preparación previa del diente y un mayor costo inicial.</li>
      </ul>
      
      <h4>Carillas de Resina (Composite)</h4>
      <p>Se modelan directamente sobre el diente en una sola sesión clínica.</p>
      <ul>
        <li><strong>Ventajas:</strong> Proceso rápido en un solo día, más económico y completamente reversible o fácil de reparar.</li>
        <li><strong>Inconvenientes:</strong> Pueden perder brillo con el tiempo y su durabilidad oscila entre 5 y 7 años.</li>
      </ul>
      
      <h3>¿Quién es Candidato para un Diseño de Sonrisa?</h3>
      <p>Es una excelente opción si deseas solucionar:</p>
      <ul>
        <li>Dientes manchados o amarillentos que no responden al blanqueamiento</li>
        <li>Bordes dentales desgastados o con pequeñas fracturas</li>
        <li>Espacios entre dientes (diastemas)</li>
        <li>Dientes levemente desalineados o de forma irregular</li>
      </ul>
      
      <h3>Paso a Paso del Tratamiento</h3>
      <ol>
        <li><strong>Evaluación y diagnóstico:</strong> Fotografías y escaneo digital de tu dentadura.</li>
        <li><strong>Diseño digital y prueba (Mock-up):</strong> Te mostramos cómo quedará tu sonrisa antes de empezar.</li>
        <li><strong>Preparación y colocación:</strong> Trabajo minucioso para adherir las carillas con precisión.</li>
      </ol>
      
      <p>En <strong>Periodent</strong> combinamos arte y tecnología para diseñar sonrisas naturales y duraderas. ¡Agenda tu evaluación estética con nosotros!</p>
    `,
    image: '/img/diseno-de-sonrisa-carillas-dentales.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-02-01',
    dateFormatted: '1 de Febrero, 2026',
    category: 'Estética Dental',
    tags: ['diseño de sonrisa', 'carillas dentales', 'porcelana', 'resina', 'estética dental']
  },
  {
    id: 12,
    slug: 'endodoncia-tratamiento-conducto-mitos-verdades',
    title: 'Endodoncia o Tratamiento de Conducto: Mitos, Verdades y Cuándo se Necesita',
    excerpt: 'Aprende todo sobre la endodoncia o tratamiento de conducto: desmentimos el mito del dolor, explicamos sus síntomas de alerta y cómo salva tu diente natural.',
    content: `
      <h2>Endodoncia: La Técnica que Salva Dientes Infectados</h2>
      <p>La endodoncia, popularmente conocida como tratamiento de conducto, es el procedimiento mediante el cual se elimina la pulpa dental infectada o inflamada (el tejido blando interno donde se encuentran los nervios y vasos sanguíneos) para desinfectar y sellar el diente.</p>
      
      <h3>Síntomas que Indican que Podrías Necesitar una Endodoncia</h3>
      <ul>
        <li><strong>Dolor agudo e intempestivo:</strong> Especialmente al masticar o al ejercer presión.</li>
        <li><strong>Sensibilidad prolongada:</strong> Molestia persistente ante bebidas o alimentos fríos y calientes.</li>
        <li><strong>Oscurecimiento del diente:</strong> Cambio de coloración debido a la necrosis pulpar.</li>
        <li><strong>Inflamación o flemón en la encía:</strong> Presencia de un pequeño bulto o pus cerca de la raíz.</li>
      </ul>
      
      <h3>Mitos Frecuentes sobre la Endodoncia</h3>
      
      <h4>Mito 1: "La endodoncia es un procedimiento doloroso"</h4>
      <p><strong>Falso.</strong> Los anestésicos locales y las técnicas actuales ayudan a controlar las molestias durante el tratamiento de conducto. La experiencia puede variar según la infección, la sensibilidad y la situación de cada paciente.</p>
      
      <h4>Mito 2: "Es mejor extraer el diente que hacer una endodoncia"</h4>
      <p><strong>Falso.</strong> Conservar la pieza dental natural siempre es la mejor opción. Mantener la raíz previene la reabsorción del hueso maxilar y el desplazamiento de los dientes vecinos.</p>
      
      <h3>Fases del Tratamiento de Conducto</h3>
      <ol>
        <li>Anestesia local y aislamiento del diente con dique de goma.</li>
        <li>Apertura de la corona e identificación de los conductos radiculares.</li>
        <li>Limpieza, limado y desinfección profunda con irrigantes antibacterianos.</li>
        <li>Obturación y sellado hermético con gutapercha.</li>
        <li>Restauración final con empaste o corona para devolver la resistencia.</li>
      </ol>
      
      <p>Si sientes dolor o molestias persistentes, en <strong>Periodent</strong> contamos con especialistas en endodoncia para eliminar el dolor y salvar tu pieza dental.</p>
    `,
    image: '/img/endodoncia-tratamiento-conducto-mitos-verdades.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-02-05',
    dateFormatted: '5 de Febrero, 2026',
    category: 'Endodoncia',
    tags: ['endodoncia', 'tratamiento de conducto', 'dolor de muela', 'salvamento dental', 'odontología']
  },
  {
    id: 13,
    slug: 'odontopediatria-cuidado-dental-ninos',
    title: 'Odontopediatría: Cómo Cuidar la Salud Dental de los Niños desde la Infancia',
    excerpt: 'Guía práctica de odontopediatría: cuándo realizar la primera visita al dentista, prevención de caries de biberón, sellantes y hábitos de higiene infantil.',
    content: `
      <h2>Cuidando las Sonrisas de los Más Pequeños</h2>
      <p>La odontopediatría es la especialidad dental encargada de la salud bucodental de bebés, niños y adolescentes. Crear hábitos saludables desde los primeros años es esencial para garantizar dientes sanos en la etapa adulta.</p>
      
      <h3>¿Por Qué son Importantes los Dientes de Leche?</h3>
      <p>A pesar de ser temporales, los dientes deciduos o "de leche" cumplen funciones fundamentales:</p>
      <ul>
        <li>Guardan el espacio para la erupción adecuada de los dientes definitivos.</li>
        <li>Permiten una correcta masticación y nutrición del niño.</li>
        <li>Facilitan el desarrollo del lenguaje y la articulación de las palabras.</li>
      </ul>
      
      <h3>¿Cuándo se Debe Realizar la Primera Visita al Dentista?</h3>
      <p>La recomendación profesional es realizar la primera revisión cuando brote el primer diente o al cumplir el primer año de edad. Esto permite evaluar el desarrollo maxilofacial y orientar a los padres en prevención.</p>
      
      <h3>Prevención de la Caries del Biberón</h3>
      <p>Ocurre por el contacto prolongado de los dientes del bebé con líquidos azucarados (leche, jugos o fórmulas). Para prevenirla:</p>
      <ul>
        <li>Evita acostar al bebé con el biberón en la boca.</li>
        <li>Limpia sus encías con una gasa húmeda antes de que broten los dientes.</li>
        <li>Inicia el cepillado con pasta fluorada adaptada a su edad tan pronto aparezca el primer diente.</li>
      </ul>
      
      <h3>Tratamientos Preventivos en Odontopediatría</h3>
      
      <h4>Sellantes de Fosas y Fisuras</h4>
      <p>Son capas protectoras delgadas que se aplican en las molares para evitar que la comida y las bacterias queden atrapadas en las grietas naturales del diente.</p>
      
      <h4>Fluorización Profesional</h4>
      <p>Aplicación de barniz de flúor para fortalecer el esmalte y hacerlo más resistente a los ácidos de las bacterias.</p>
      
      <p>En <strong>Periodent</strong> ofrecemos un ambiente cálido y sin miedos para tus hijos. ¡Haz de su salud dental una experiencia positiva!</p>
    `,
    image: '/img/odontopediatria-cuidado-dental-ninos.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-02-10',
    dateFormatted: '10 de Febrero, 2026',
    category: 'Odontopediatría',
    tags: ['odontopediatría', 'dientes de leche', 'salud bucal infantil', 'sellantes', 'prevención']
  },
  {
    id: 14,
    slug: 'blanqueamiento-dental-profesional-vs-casero',
    title: 'Blanqueamiento Dental Profesional vs. Remedios Caseros: Riesgos y Resultados',
    excerpt: 'Analizamos la diferencia entre un blanqueamiento dental clínico seguro y los peligrosos remedios caseros como el carbón activado o el bicarbonato.',
    content: `
      <h2>Recupera el Brillo y Blanco de tus Dientes</h2>
      <p>Tener una sonrisa brillante es el deseo de muchas personas. Sin embargo, con el paso del tiempo, bebidas como el café, té, vino tinto o el tabaco manchan y amarillean el esmalte dental.</p>
      
      <h3>Blanqueamiento Dental Profesional en Clínica</h3>
      <p>El tratamiento clínico utiliza geles blanqueadores de uso exclusivo profesional (peróxido de hidrógeno o de carbamida en concentraciones controladas).</p>
      <ul>
        <li><strong>En Clínica (Luz LED/Láser):</strong> Resultados inmediatos en una sesión de aproximadamente 45 minutos.</li>
        <li><strong>Ambulatorio Acompañado:</strong> Cubetas personalizadas que el paciente usa en casa bajo estricta supervisión del odontólogo.</li>
        <li><strong>Seguridad:</strong> Se aplican protectores especiales en las encías para prevenir irritaciones o quemaduras químicas.</li>
      </ul>
      
      <h3>Peligros de los Remedios Caseros y Populares</h3>
      <p>Existen múltiples recetas en internet que prometen blanquear los dientes pero que causan daños irreversibles:</p>
      <ul>
        <li><strong>Carbón Activado y Bicarbonato de Sodio:</strong> Son polvos altamente abrasivos. No blanquean el diente por dentro; desgastan la capa protectora de esmalte, dejando expuesta la dentina (que es más amarilla).</li>
        <li><strong>Zumo de Limón o Vinagre:</strong> Son ácidos agresivos que desmineralizan el diente, provocando porosidad y aumentando la sensibilidad extrema.</li>
      </ul>
      
      <h3>¿Cómo Manejar la Sensibilidad Temporal?</h3>
      <p>El blanqueamiento profesional puede causar una leve sensibilidad pasajera durante las primeras 24-48 horas. Se controla fácilmente con desensibilizantes e higiene suave.</p>
      
      <p>No arriesgues la salud de tu esmalte. En <strong>Periodent</strong> evaluamos tu dentadura para valorar si el blanqueamiento es adecuado y explicarte sus resultados esperables y cuidados.</p>
    `,
    image: '/img/blanqueamiento-dental-profesional-vs-casero.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-02-14',
    dateFormatted: '14 de Febrero, 2026',
    category: 'Estética Dental',
    tags: ['blanqueamiento dental', 'estética', 'sensibilidad dental', 'remedios caseros', 'sonrisa blanca']
  },
  {
    id: 15,
    slug: 'periodontitis-gingivitis-sangrado-encias-tratamiento',
    title: 'Gingivitis y Periodontitis: Causas, Sangrado de Encías y Cómo Prevenir la Pérdida Dental',
    excerpt: '¿Te sangran las encías al cepillarte? Conoce las diferencias entre gingivitis y periodontitis, los síntomas de alerta y los tratamientos periodontales.',
    content: `
      <h2>La Salud de tus Encías es el Cimiento de tu Sonrisa</h2>
      <p>Las enfermedades periodontales afectan los tejidos que sostienen las piezas dentales. Inician silenciosamente y, de no tratarse, son la causa principal de pérdida de dientes en adultos.</p>
      
      <h3>Gingivitis: La Etapa Incial y Reversible</h3>
      <p>Es la inflamación superficial de la encía provocada por la acumulación de placa bacteriana y sarro en el borde gingival.</p>
      <ul>
        <li><strong>Síntomas:</strong> Encías rojas, hinchadas y sangrado al cepillarse o usar hilo dental.</li>
        <li><strong>Tratamiento:</strong> En muchos casos mejora con una limpieza profesional y una corrección de las técnicas de cepillado, según la valoración clínica.</li>
      </ul>
      
      <h3>Periodontitis: La Etapa Avanzada e Irreversible</h3>
      <p>Si la gingivitis no se trata, las bacterias avanzan por debajo de la encía, destruyendo el hueso alveolar y el ligamento periodontal que sujeta al diente.</p>
      <ul>
        <li><strong>Síntomas:</strong> Retracción de encías (dientes más largos), bolsas periodontales con pus, mal aliento persistente (halitosis) y movilidad o separación de dientes.</li>
        <li><strong>Tratamiento:</strong> Requiere raspado y alisado radicular (curetaje periodontal) para desinfectar las raíces.</li>
      </ul>
      
      <h3>Factores que Aumentan el Riesgo</h3>
      <ul>
        <li>Tabaquismo (el cigarrillo reduce el riego sanguíneo y oculta el sangrado)</li>
        <li>Diabetes no controlada</li>
        <li>Cambios hormonales (embarazo o menorquía)</li>
        <li>Estrés y predisposición genética</li>
      </ul>
      
      <p>Si notas sangrado o inflamación en tus encías, consulta a los especialistas en periodoncia de <strong>Periodent</strong> para frenar el avance de la enfermedad.</p>
    `,
    image: '/img/periodontitis-gingivitis-sangrado-encias-tratamiento.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-02-18',
    dateFormatted: '18 de Febrero, 2026',
    category: 'Periodoncia',
    tags: ['periodoncia', 'gingivitis', 'periodontitis', 'sangrado de encías', 'curetaje dental']
  },
  {
    id: 16,
    slug: 'bruxismo-sintomas-placas-miorrelajantes',
    title: 'Bruxismo (Chirrido de Dientes): Causas, Síntomas y Uso de Placas Miorrelajantes',
    excerpt: 'Descubre si sufres de bruxismo nocturno o diurno, sus efectos en tus dientes y articulación (ATM), y cómo las férulas miorrelajantes protegen tu sonrisa.',
    content: `
      <h2>¿Apretas o Rechina los Dientes Involuntariamente?</h2>
      <p>El bruxismo es una patología caracterizada por apretar (bruxismo céntrico) o rechinar (bruxismo excéntrico) las estructuras dentales sin un fin funcional. Afecta tanto a adultos como a niños, y suele intensificarse durante la noche.</p>
      
      <h3>Principales Causas del Bruxismo</h3>
      <p>El factor desencadenante más frecuente es el estrés emocional, la ansiedad y el ritmo de vida acelerado. También puede estar relacionado con maloclusiones o problemas en la mordida.</p>
      
      <h3>Síntomas que Debes Identificar</h3>
      <ul>
        <li>Dolor o tensión en la mandíbula y cuello al despertar</li>
        <li>Dolores de cabeza matutinos frecuentes (cefaleas tensionales)</li>
        <li>Desgaste o aplanamiento notable en las superficies de los dientes</li>
        <li>Sensibilidad dental generalizada por pérdida de esmalte</li>
        <li>Chasquidos o ruidos en la articulación temporomandibular (ATM)</li>
      </ul>
      
      <h3>Tratamiento: Placas Miorrelajantes o Férulas de Descarga</h3>
      <p>El tratamiento principal consiste en la confección a medida de una férula miorrelajante de acrílico rígido.</p>
      <ul>
        <li><strong>Protección mecánica:</strong> Evita el contacto directo diente con diente, previniendo fracturas y desgaste.</li>
        <li><strong>Relajación muscular:</strong> Disminuye la sobrecarga de los músculos masticatorios y descomprime la articulación temporomandibular.</li>
      </ul>
      
      <h3>Recomendaciones Adicionales</h3>
      <p>Combinar la férula con técnicas de relajación, masajes faciales suaves antes de dormir y fisioterapia maxilofacial en casos severos ayuda significativamente.</p>
      
      <p>En <strong>Periodent</strong> confeccionamos placas miorrelajantes personalizadas de alta precisión para proteger tu salud bucal.</p>
    `,
    image: '/img/bruxismo-sintomas-placas-miorrelajantes.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-02-22',
    dateFormatted: '22 de Febrero, 2026',
    category: 'Odontología General',
    tags: ['bruxismo', 'férula de descarga', 'placa miorrelajante', 'dolor de mandíbula', 'ATM']
  },
  {
    id: 17,
    slug: 'muelas-del-juicio-extraccion-terceros-molares',
    title: 'Muelas del Juicio (Terceros Molares): ¿Cuándo es Necesario Extraerlas?',
    excerpt: 'Aprende cuándo se debe realizar la extracción de las muelas del juicio retenidas o impactadas, cómo es la cirugía y recomendaciones de recuperación.',
    content: `
      <h2>Todo sobre los Terceros Molares</h2>
      <p>Las muelas del juicio son los últimos dientes en salir, apareciendo habitualmente entre los 17 y 25 años. A menudo, las mandíbulas modernas no tienen suficiente espacio para albergarlas correctamente.</p>
      
      <h3>¿Cuándo es Necesaria la Extracción de la Muela del Juicio?</h3>
      <p>No siempre se deben extraer. Sin embargo, la intervención está indicada en los siguientes casos:</p>
      <ul>
        <li><strong>Muelas retenidas o impactadas:</strong> No logran erupcionar y quedan atrapadas debajo de la encía o hueso.</li>
        <li><strong>Pericoronaritis:</strong> Infección e inflamación dolorosa de la encía alrededor de la muela semierupcionada.</li>
        <li><strong>Daño a dientes adyacentes:</strong> La muela empuja el segundo molar, provocando caries o reabsorción de su raíz.</li>
        <li><strong>Quistes mandibulares:</strong> Acumulación de líquido alrededor del folículo del diente no erupcionado.</li>
      </ul>
      
      <h3>¿Cómo es el Procedimiento Quirúrgico?</h3>
      <p>Es una cirugía oral menor efectuada bajo anestesia local. En <strong>Periodent</strong> utilizamos técnicas mínimamente invasivas para garatizar un procedimiento indoloro y seguro.</p>
      
      <h3>Cuidados Clave Durante la Recuperación</h3>
      <ol>
        <li>Morder la gasa de compresión durante 30 a 45 minutos tras la extracción.</li>
        <li>Aplicar compresas frías en la mejilla las primeras 24 horas.</li>
        <li>Mantener una dieta blanda y tibia o fría (helados, gelatinas, sopas frías).</li>
        <li>Evitar escupir, fumar o usar pajillas/popotes para no desprender el coágulo cicatrizante.</li>
        <li>Tomar los analgésicos y antibióticos recetados por el cirujano.</li>
      </ol>
      
      <p>¿Sientes molestias en la zona posterior de tus maxilares? Agenda una evaluación radiográfica en <strong>Periodent</strong>.</p>
    `,
    image: '/img/muelas-del-juicio-extraccion-terceros-molares.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-02-26',
    dateFormatted: '26 de Febrero, 2026',
    category: 'Cirugía Oral',
    tags: ['muelas del juicio', 'terceros molares', 'cirugía oral', 'extracción dental', 'recuperación']
  },
  {
    id: 18,
    slug: 'sensibilidad-dental-causas-y-tratamiento',
    title: 'Sensibilidad Dental: Por Qué Ocurre y Tratamientos Eficaces para Eliminar el Dolor',
    excerpt: '¿Sientes un chispazo de dolor al comer o beber algo frío o caliente? Explicamos las causas de la hipersensibilidad dentinaria y cómo curarla.',
    content: `
      <h2>¿Qué Causa el Dolor o Pinchazo por Sensibilidad?</h2>
      <p>La hipersensibilidad dentinaria es un dolor intenso y transitorio provocado por la exposición de la dentina (la capa interna del diente protegida normalmente por el esmalte y la encía) ante estímulos fríos, calientes, dulces o al cepillarse.</p>
      
      <h3>Causas Principales de la Exposición Dentinaria</h3>
      <ul>
        <li><strong>Cepillado dental agresivo:</strong> Usar cepillos de cerdas duras o ejercer demasiada presión destruye el esmalte.</li>
        <li><strong>Recesión gingival:</strong> Desplazamiento de las encías que deja al descubierto las raíces dentales.</li>
        <li><strong>Consumo de alimentos ácidos:</strong> Cítricos, refrescos y bebidas energéticas que desgastan el esmalte.</li>
        <li><strong>Bruxismo:</strong> El rechinamiento continuo desgasta las capas protectoras del diente.</li>
        <li><strong>Caries o arreglos defectuosos:</strong> Grietas que permiten la entrada de estímulos hacia el nervio.</li>
      </ul>
      
      <h3>¿Cómo se Elimina la Sensibilidad Dental?</h3>
      
      <h4>Tratamiento en Casa</h4>
      <p>Usar dentífricos y enjuagues desensibilizantes específicos de forma continuada. Contienen nitrato de potasio que bloquea la transmisión del impulso nervioso al cerebro.</p>
      
      <h4>Tratamiento Profesional en Clínica</h4>
      <ul>
        <li>Aplicación de barnices concentrados de flúor para sellar los túbulos dentinarios.</li>
        <li>Uso de selladores o resinas en las zonas expuestas del cuello del diente.</li>
        <li>Injertos mucogingivales en casos de retracción severa de encía.</li>
      </ul>
      
      <p>No te acostumbres a vivir con dolor al comer o beber. En <strong>Periodent</strong> identificamos la causa exacta de tu sensibilidad para ofrecerte la solución adecuada.</p>
    `,
    image: '/img/sensibilidad-dental-causas-y-tratamiento.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-03-01',
    dateFormatted: '1 de Marzo, 2026',
    category: 'Prevención',
    tags: ['sensibilidad dental', 'dentina', 'esmalte dental', 'dolor con frío', 'prevención']
  },
  {
    id: 19,
    slug: 'halitosis-mal-aliento-causas-soluciones',
    title: 'Halitosis (Mal Aliento): Causas Ocultas y Soluciones Odontológicas Definitivas',
    excerpt: 'Descubre qué provoca realmente la halitosis o mal aliento, por qué los caramelos o enjuagues no son suficientes y cómo solucionarlo en la consulta.',
    content: `
      <h2>Supera el Problema de la Halitosis de Forma Definitiva</h2>
      <p>El mal aliento o halitosis es una afección vergonzosa que afecta las relaciones sociales de muchas personas. En más del 85% a 90% de los casos, la causa se encuentra directamente en la cavidad bucal.</p>
      
      <h3>Principales Causas Buco-Dentales</h3>
      <ul>
        <li><strong>Acumulación de saburra en la lengua:</strong> Capa de bacterias y restos de comida alojados en la parte posterior de la lengua.</li>
        <li><strong>Enfermedades periodontales:</strong> La gingivitis y periodontitis albergan bacterias anaerobias que liberan compuestos volátiles de azufre (CVS) de mal olor.</li>
        <li><strong>Caries no tratadas:</strong> Cavidades donde se pudren restos de alimentos.</li>
        <li><strong>Boca seca (Xerostomía):</strong> La falta de saliva impide la autolimpieza natural de la boca.</li>
      </ul>
      
      <h3>¿Por Qué las Pastillas o Menta No Funcionan?</h3>
      <p>Los caramelos, chicles y enjuagues comerciales comunes solo enmascaran el olor durante unos minutos, pero no eliminan las bacterias que generan la fermentación.</p>
      
      <h3>Solución Odontológica Integral</h3>
      <ol>
        <li><strong>Limpieza profesional:</strong> Eliminación de todo el sarro acumulado por encima y por debajo de las encías.</li>
        <li><strong>Limpieza lingual adecuada:</strong> Incorporación del uso diario del raspador o limpiador lingual.</li>
        <li><strong>Tratamiento de focos infecciosos:</strong> Curación de caries, coronas desajustadas o endodoncias.</li>
        <li><strong>Hidratación y enjuagues antisépticos específicos:</strong> Uso de productos con clorhexidina o cloruro de cetilpiridinio bajo prescripción.</li>
      </ol>

      <p>En <strong>Periodent</strong> abordamos la halitosis de manera confidencial y profesional para devolverte la seguridad en tu aliento.</p>
    `,
    image: '/img/halitosis-mal-aliento-causas-soluciones.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-03-03',
    dateFormatted: '3 de Marzo, 2026',
    category: 'Prevención',
    tags: ['halitosis', 'mal aliento', 'limpieza de lengua', 'higiene bucal', 'bacterias']
  },
  {
    id: 20,
    slug: 'cuidados-post-ortodoncia-importancia-retenedores',
    title: 'Cuidados Post-Ortodoncia: Por Qué los Retenedores son Clave para Mantener tu Sonrisa',
    excerpt: 'Terminaste tu tratamiento de brackets o alineadores? Descubre por qué el uso de retenedores fijos o removibles es vital para evitar la recidiva dental.',
    content: `
      <h2>Protege los Resultados de tu Tratamiento de Ortodoncia</h2>
      <p>Retirar los brackets o completar el último alineador transparente es un momento emocionante. Sin embargo, para garantizar que tus dientes permanezcan en su lugar perfecto, entra en juego la fase de retención.</p>
      
      <h3>¿Por Qué se Mueven los Dientes Después de la Ortodoncia?</h3>
      <p>Los dientes tienen "memoria". Las fibras del ligamento periodontal y el hueso circundante necesitan tiempo para reorganizarse y consolidarse en la nueva posición. Sin retención, existe un alto riesgo de recidiva (que los dientes vuelvan a torcerse).</p>
      
      <h3>Tipos de Retenedores Ortodónticos</h3>
      
      <h4>1. Retenedores Fijos</h4>
      <p>Consisten en un alambre fino de acero inoxidable adherido a la cara interna de los dientes anteriores (generalmente de canino a canino inferior).</p>
      <ul>
        <li><strong>Ventaja:</strong> Imperceptible, cómodo y trabaja las 24 horas del día.</li>
      </ul>
      
      <h4>2. Retenedores Removibles (Placas Transparentes / Essix o Hawley)</h4>
      <p>Fundas transparentes hechas a medida que cubren toda la arcada dental.</p>
      <ul>
        <li><strong>Uso recomendado:</strong> Al principio durante todo el día (retirándolos para comer), y posteriormente solo durante las noches.</li>
      </ul>
      
      <h3>Mantenimiento e Higiene de los Retenedores</h3>
      <ul>
        <li>Lava los retenedores removibles diariamente con agua fría y jabón neutro (nunca agua caliente ni pasta dental abrasiva).</li>
        <li>Utiliza hilo dental especial (superfloss) para limpiar entre las piezas con retenedor fijo.</li>
        <li>Guárdalos siempre en su estuche rígido cuando no los estés usando para evitar pérdidas o roturas.</li>
        <li>Asiste a tus revisiones de control post-ortodoncia en la clínica.</li>
      </ul>
      
      <p>Mantén tu sonrisa perfecta de por vida. En <strong>Periodent</strong> cuidamos cada etapa de tu tratamiento ortodóntico.</p>
    `,
    image: '/img/cuidados-post-ortodoncia-importancia-retenedores.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-03-05',
    dateFormatted: '5 de Marzo, 2026',
    category: 'Ortodoncia',
    tags: ['ortodoncia', 'retenedores dentales', 'post-ortodoncia', 'retenedor fijo', 'retenedor essix']
  },
  {
    id: 21,
    slug: 'como-elegir-dentista-en-biscucuy',
    title: 'Cómo elegir un dentista en Biscucuy: guía para tu primera consulta',
    excerpt: 'Qué revisar antes de elegir un dentista en Biscucuy: ubicación, servicios, evaluación inicial, comunicación y preguntas importantes.',
    content: `
      <h2>Encontrar un dentista en Biscucuy</h2>
      <p>Elegir una clínica dental no debería depender únicamente de la cercanía o del precio. Una primera consulta es una oportunidad para conocer cómo se evaluará tu caso, qué alternativas existen y qué información necesitas antes de decidir un tratamiento.</p>

      <h3>1. Comprueba la ubicación y los canales de contacto</h3>
      <p>Una clínica cercana facilita las revisiones, especialmente cuando un tratamiento requiere varias visitas. Antes de acudir, confirma la dirección, el horario, el teléfono y la forma de solicitar una cita.</p>

      <h3>2. Pregunta qué incluye la evaluación</h3>
      <p>La evaluación puede variar según el motivo de consulta. Pregunta si incluye revisión clínica, fotografías, radiografías u otros estudios, y si esos procedimientos tienen un costo adicional. El diagnóstico debe realizarlo un profesional después de valorar tu situación.</p>

      <h3>3. Solicita una explicación clara</h3>
      <p>Un buen proceso de atención debe permitirte entender el problema, las alternativas disponibles, la duración aproximada, los cuidados y el presupuesto. También tienes derecho a preguntar qué riesgos y limitaciones tiene cada opción.</p>

      <h3>4. Lleva información útil a la cita</h3>
      <ul>
        <li>Lista de medicamentos que utilizas.</li>
        <li>Antecedentes de alergias o enfermedades importantes.</li>
        <li>Radiografías recientes, si las tienes.</li>
        <li>Una lista de tus dudas y molestias.</li>
      </ul>

      <h3>Atención dental en Biscucuy</h3>
      <p>Periodent cuenta con una sede en Biscucuy, en la Carrera 1 Bolívar, entre calle Páez y Negro Primero, al lado de la antigua Casa Blanca. Puedes revisar la información de la <a href="/ubicacion/biscucuy">sede de Biscucuy</a> o <a href="/contacto">solicitar información</a> antes de acudir.</p>

      <p>Este artículo es informativo. La recomendación de un tratamiento depende de la evaluación clínica individual.</p>
    `,
    image: '/img/como-elegir-dentista-en-biscucuy.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-09-08',
    dateFormatted: '8 de Septiembre, 2026',
    category: 'Clínica',
    tags: ['dentista en Biscucuy', 'clínica dental Biscucuy', 'primera consulta', 'salud bucal']
  },
  {
    id: 22,
    slug: 'como-prepararse-para-cita-dental-en-chabasquen',
    title: 'Cómo prepararte para una cita dental en Chabasquén',
    excerpt: 'Consejos prácticos para preparar tu primera cita dental en Chabasquén, organizar tus antecedentes y aprovechar mejor la evaluación.',
    content: `
      <h2>Antes de tu cita dental en Chabasquén</h2>
      <p>Prepararte con anticipación puede ayudarte a explicar mejor tus síntomas y a aprovechar el tiempo de la consulta. No necesitas hacer nada complicado: basta con reunir información y anotar tus preguntas.</p>

      <h3>Qué información conviene llevar</h3>
      <ul>
        <li>Motivo principal de la consulta y desde cuándo comenzó.</li>
        <li>Medicamentos, alergias y antecedentes médicos relevantes.</li>
        <li>Tratamientos dentales anteriores y radiografías disponibles.</li>
        <li>Datos de contacto para recibir indicaciones o confirmar controles.</li>
      </ul>

      <h3>Preguntas que puedes hacer</h3>
      <p>Es normal tener dudas sobre un diagnóstico o tratamiento. Puedes preguntar cuáles son las alternativas, cuánto tiempo podría durar el proceso, qué cuidados requiere, qué controles serán necesarios y cómo se calcula el presupuesto.</p>

      <h3>Si tienes ansiedad o miedo al dentista</h3>
      <p>Informa al equipo antes de comenzar. Explicar tus preocupaciones permite organizar la consulta paso a paso y acordar pausas o señales de comunicación. No ocultes información importante por vergüenza: ayuda a que la atención sea más segura.</p>

      <h3>Dónde solicitar atención odontológica en Chabasquén</h3>
      <p>Periodent atiende en la Avenida Sucre con calle Córdoba y Arismendi, edificio del Señor Chelopez, planta baja, local 5, Chabasquén, Portuguesa. Consulta la página de <a href="/ubicacion/chabasquen">ubicación y horarios en Chabasquén</a> o comunícate mediante la <a href="/contacto">página de contacto</a>.</p>

      <p>La información de este artículo es educativa y no reemplaza una valoración profesional.</p>
    `,
    image: '/img/como-prepararse-para-cita-dental-en-chabasquen.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-09-07',
    dateFormatted: '7 de Septiembre, 2026',
    category: 'Clínica',
    tags: ['dentista en Chabasquén', 'cita dental Chabasquén', 'clínica dental', 'Portuguesa']
  },
  {
    id: 23,
    slug: 'que-hacer-ante-una-urgencia-dental-en-portuguesa',
    title: 'Qué hacer ante una urgencia dental en Biscucuy o Chabasquén',
    excerpt: 'Orientación general ante dolor intenso, golpes, sangrado o una pieza dental dañada mientras consigues atención odontológica.',
    content: `
      <h2>Identifica una posible urgencia dental</h2>
      <p>Un dolor intenso, una inflamación que aumenta, un golpe en la boca, sangrado persistente o una pieza dental fracturada requieren atención profesional. La urgencia puede tener causas distintas y no es posible determinarla solo por internet.</p>

      <h3>Qué puedes hacer mientras solicitas atención</h3>
      <ul>
        <li>Enjuaga suavemente la boca con agua si existe suciedad o restos.</li>
        <li>Aplica frío externo protegido con una tela si hay inflamación por un golpe.</li>
        <li>Si se desprendió una pieza, guárdala sin limpiarla con productos químicos y solicita orientación.</li>
        <li>Evita colocar aspirina, alcohol, sustancias irritantes o remedios caseros directamente sobre la encía.</li>
        <li>No tomes antibióticos por cuenta propia.</li>
      </ul>

      <h3>Cuándo acudir a un servicio de emergencia</h3>
      <p>Busca atención inmediata si tienes dificultad para respirar o tragar, inflamación que compromete la cara o el cuello, sangrado que no se detiene, fiebre alta, pérdida de conciencia o un traumatismo importante.</p>

      <h3>Atención dental en Portuguesa</h3>
      <p>Si la situación no presenta los signos de alarma anteriores, puedes comunicarte con una clínica dental de Biscucuy o Chabasquén para conocer la disponibilidad de evaluación. Periodent publica sus <a href="/ubicacion">ubicaciones y horarios</a> y atiende solicitudes mediante <a href="/contacto">contacto directo</a>.</p>

      <p>Este contenido ofrece orientación general y no sustituye la valoración de un odontólogo ni la atención de emergencias.</p>
    `,
    image: '/img/que-hacer-ante-una-urgencia-dental-en-portuguesa.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-09-06',
    dateFormatted: '6 de Septiembre, 2026',
    category: 'Urgencias dentales',
    tags: ['urgencia dental', 'dentista Portuguesa', 'dolor de muela', 'Biscucuy', 'Chabasquén']
  },
  {
    id: 24,
    slug: 'cada-cuanto-hacer-limpieza-dental-en-portuguesa',
    title: '¿Cada cuánto hacer una limpieza dental en Portuguesa?',
    excerpt: 'Conoce qué factores influyen en la frecuencia de una limpieza dental y cuándo conviene consultar a un odontólogo en Portuguesa.',
    content: `
      <h2>La frecuencia de una limpieza dental depende de cada persona</h2>
      <p>La limpieza dental profesional ayuda a retirar acumulaciones que no siempre se eliminan con el cepillado. La frecuencia adecuada depende de la higiene diaria, la salud de las encías, el consumo de tabaco, algunas enfermedades y la tendencia a acumular sarro.</p>

      <h3>Orientación general</h3>
      <p>Muchas personas reciben una recomendación de control periódico, pero no existe un intervalo único para todos. Un paciente con inflamación de encías o antecedentes de enfermedad periodontal puede necesitar revisiones más frecuentes que alguien sin esos factores.</p>

      <h3>Señales para pedir una evaluación</h3>
      <ul>
        <li>Sangrado al cepillarte o usar hilo dental.</li>
        <li>Mal aliento que persiste pese a la higiene.</li>
        <li>Sensibilidad, dolor o cambios en una pieza dental.</li>
        <li>Sarro visible o manchas que no desaparecen con el cepillado.</li>
        <li>Encías inflamadas, retraídas o con cambios de color.</li>
      </ul>

      <h3>Cómo es una consulta de limpieza dental</h3>
      <p>El profesional revisa el estado de la boca, identifica depósitos de placa o sarro y determina qué procedimiento es apropiado. Después puede recomendar cambios de higiene y una fecha de control adaptada a tus necesidades.</p>

      <h3>Limpieza dental en Biscucuy y Chabasquén</h3>
      <p>Periodent ofrece información sobre sus servicios de <a href="/servicios">atención odontológica</a> y sus sedes en Portuguesa. Puedes <a href="/contacto">solicitar una cita</a> para conocer qué frecuencia resulta adecuada en tu caso.</p>

      <p>La recomendación final debe realizarla un profesional después de una evaluación clínica.</p>
    `,
    image: '/img/cada-cuanto-hacer-limpieza-dental-en-portuguesa.png',
    author: 'Equipo editorial de Periodent',
    date: '2026-09-05',
    dateFormatted: '5 de Septiembre, 2026',
    category: 'Prevención',
    tags: ['limpieza dental Portuguesa', 'limpieza dental Biscucuy', 'salud bucal', 'prevención']
  }
]

// Función para obtener todos los posts
export function getAllPosts() {
  return blogPosts.sort((a, b) => new Date(b.date) - new Date(a.date))
}

// Función para obtener un post por slug
export function getPostBySlug(slug) {
  return blogPosts.find(post => post.slug === slug)
}

// Función para obtener posts relacionados
export function getRelatedPosts(currentSlug, limit = 2) {
  return blogPosts
    .filter(post => post.slug !== currentSlug)
    .slice(0, limit)
}

// Función para obtener categorías únicas
export function getCategories() {
  const categories = [...new Set(blogPosts.map(post => post.category))]
  return categories
}
